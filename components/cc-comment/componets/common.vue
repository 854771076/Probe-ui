<template>
	<view class="comment_item">
		<view class="top">
			<view class="top_left">
				<img class="user_avatar" :src="getHeader(props.data.photo)" />
				<uni-tag v-if="props.data.is_mine" class="tag" type="primary" :inverted="false" text="作者" size="mini"
					circle />
				<span class="user_name">{{ props.data.username }}</span>
	...
	<view class="content" @tap="replyClick(props.data)">
      {{ c_content }}
		...
		<view class="bottom">
			<span class="create_time">{{ timeformet(props.data.create_time) }}</span>
			<span v-if="props.data.is_mine" class="delete" @tap="deleteClick(props.data)">删除</span>
